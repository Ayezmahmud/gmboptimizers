...
const CountrySelector = () => {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        <img src={`https://flagcdn.com/w40/${countryCode === 'uk' ? 'gb' : countryCode}.png`} alt={country.name} className="w-5 h-3.5 object-cover rounded-[2px]" />
      ...
      {open && (
        <div className="absolute right-0 top-full mt-1 bg-background border border-border shadow-xl z-50 min-w-[180px] py-1">
          {countries.map((c) => (
            ...
            >
              <img src={`https://flagcdn.com/w40/${c.code === 'uk' ? 'gb' : c.code}.png`} alt={c.name} className="w-5 h-3.5 object-cover rounded-[2px]" />
